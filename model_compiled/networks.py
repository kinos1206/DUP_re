...
def define_gridnet(input_nc, output_nc, ngf, use_dropout=False, gpu_ids=[]):
    gridnet = None
    noncompile_gridnet = None
    use_gpu = len(gpu_ids) > 0

    if use_gpu:
        assert(torch.cuda.is_available())

    noncompile_gridnet = GridNet_Network(input_nc, output_nc, ngf, use_dropout=use_dropout, gpu_ids=gpu_ids)
    gridnet = torch.compile(noncompile_gridnet)
...
def define_pixelnet(input_nc, output_nc, ngf, use_dropout=False, gpu_ids=[]):
    pixelnet = None
    noncompile_pixelnet = None
    use_gpu = len(gpu_ids) > 0

    if use_gpu:
        assert(torch.cuda.is_available())

    noncompile_pixelnet = PixelNet_Network(input_nc, output_nc, ngf, use_dropout=use_dropout, gpu_ids=gpu_ids)
    pixelnet = torch.compile(noncompile_pixelnet)
...
def define_depixelnet(input_nc, output_nc, ngf, use_dropout=False, gpu_ids=[]):
    depixelnet = None
    noncompile_depixelnet = None
    use_gpu = len(gpu_ids) > 0

    if use_gpu:
        assert(torch.cuda.is_available())

    noncompile_depixelnet = DepixelNet_Network(input_nc, output_nc, ngf, use_dropout=use_dropout, gpu_ids=gpu_ids)
    depixelnet = torch.compile(noncompile_depixelnet)
