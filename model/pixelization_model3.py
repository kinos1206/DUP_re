...
class PixelizationModel():
    ...
    def initialize(self, opt):
        ...
        self.upsampling = torch.hub.load(
                "./nunif", "waifu2x", keep_alpha=False,
                model_type="swin_unet/art", method="scale2x", noise_level=3,
                source="local", trust_repo=True).cuda(self.gpu_ids)
    ...
    def change_size(self, img, size=None, ratio=None, flag=False):
        if size:
            if flag:
                '''
                a,b = size
                img = torch.nn.functional.interpolate(img, size=(int(a/2), int(b/2)), mode='bilinear')
                img = img.squeeze()
                img = (img + 1) / 2.0               
                img = self.upsampling.infer(img)
                img = img.unsqueeze(0)
                '''
    ...
    def backward_all_net(self):
        ...
        '''
        fake_B_small_x8_gradient_x1 = self.get_gradient(fake_B_bi_x8)
        fake_B_small_x6_gradient_x1 = self.get_gradient(fake_B_bi_x6)
        fake_B_small_x4_gradient_x1 = self.get_gradient(fake_B_bi_x4)
        real_A_gradient_x1 = self.get_gradient(self.real_A.float())
        ...
        self.count += 1
        if(self.count == 1):
            self.save_img(real_A_small_x4_gradient_x,'real_A_gradient_x')
            self.save_img(real_A_small_x4_gradient_y,'real_A_gradient_y')
            ...
            self.save_img(fake_B_small_x4_gradient_x,'fake_B_small_x4_gradient_x')
            img_gray = self.getGrayImage(self.real_A)
            self.save_img(img_gray,'real_A_gray')
            img_gray = self.getGrayImage(self.real_A_small_x8)
            self.save_img(img_gray,'real_A_small_x8_gray')
            img_gray = self.getGrayImage(self.real_A_small_x6)
            self.save_img(img_gray,'real_A_small_x6_gray')
            img_gray = self.getGrayImage(self.real_A_small_x4)
            self.save_img(img_gray,'real_A_small_x4_gray')
            sys.exit()
        '''
